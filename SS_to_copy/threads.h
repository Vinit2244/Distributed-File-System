...
extern pthread_mutex_t* file_mutex_arr;             // Used to lock a particular file path when some client is performing write operation on it, so that no two clients can write on the same file at the same time
