...
void *handle_create(request req, int client_id)
{
    ...
    else
    {
        ...
        if (found_server->is_backedup == 1)
        {
            ...
            send(s_fd, r, sizeof(st_request), 0);
            int logging = insert_log(SS, found_server->ssid, atoi(found_server->backup_port[0]), r->request_type, r->data, OK);
            if (logging == 0)
            {
                printf(RED("Logging not added\n"));
            }
            ...
            send(s_fd, r, sizeof(st_request), 0);
            int logging = insert_log(SS, found_server->ssid, atoi(found_server->backup_port[1]), r->request_type, r->data, OK);
            if (logging == 0)
            {
                printf(RED("Logging not added\n"));
            }
    ...
    else
    {
        ...
        if (found_server->status == 1)
        {
            ...
            if (found_server->is_backedup == 1)
            {
                ...
                s_fd = connect_to_port(found_server->backup_port[1]);
                send(s_fd, r, sizeof(st_request), 0);
                logging = insert_log(SS, found_server->ssid, atoi(found_server->backup_port[1]), r->request_type, r->data, OK);
                if (logging == 0)
                {
                    printf(RED("Logging not added\n"));
                }
